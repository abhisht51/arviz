(function() {
  var fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      var force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      var element = document.getElementById("0d694408-e156-4e9d-83bc-32e6e30aaf1d");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid '0d694408-e156-4e9d-83bc-32e6e30aaf1d' but no matching script tag was found.")
        }
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error() {
          console.error("failed to load " + url);
        }
    
        for (var i = 0; i < css_urls.length; i++) {
          var url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error;
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        const hashes = {"https://cdn.bokeh.org/bokeh/release/bokeh-2.2.1.min.js": "qkRvDQVAIfzsJo40iRBbxt6sttt0hv4lh74DG7OK4MCHv4C5oohXYoHUM5W11uqS", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.2.1.min.js": "Sb7Mr06a9TNlet/GEBeKaf5xH3eb6AlCzwjtU82wNPyDrnfoiVl26qnvlKjmcAd+", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.2.1.min.js": "HaJ15vgfmcfRtB4c4YBOI4f1MUujukqInOWVqZJZZGK7Q+ivud0OKGSTn/Vm2iso"};
    
        for (var i = 0; i < js_urls.length; i++) {
          var url = js_urls[i];
          var element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error;
          element.async = false;
          element.src = url;
          if (url in hashes) {
            element.crossOrigin = "anonymous";
            element.integrity = "sha384-" + hashes[url];
          }
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      
      var js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.2.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.2.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.2.1.min.js"];
      var css_urls = [];
      
    
      var inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          (function() {
            var fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  var docs_json = '{"fcd226ef-5b5c-419d-85da-7c0ee6316825":{"roots":{"references":[{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]}},"selected":{"id":"65310"},"selection_policy":{"id":"65309"}},"id":"65293","type":"ColumnDataSource"},{"attributes":{},"id":"65310","type":"Selection"},{"attributes":{"formatter":{"id":"65307"},"ticker":{"id":"65267"}},"id":"65266","type":"LinearAxis"},{"attributes":{},"id":"65311","type":"UnionRenderers"},{"attributes":{},"id":"65307","type":"BasicTickFormatter"},{"attributes":{"overlay":{"id":"65283"}},"id":"65278","type":"LassoSelectTool"},{"attributes":{"source":{"id":"65293"}},"id":"65297","type":"CDSView"},{"attributes":{"below":[{"id":"65266"}],"center":[{"id":"65269"},{"id":"65273"},{"id":"65315"}],"left":[{"id":"65270"}],"output_backend":"webgl","plot_height":500,"plot_width":500,"renderers":[{"id":"65296"},{"id":"65301"},{"id":"65320"}],"title":{"id":"65322"},"toolbar":{"id":"65284"},"toolbar_location":null,"x_range":{"id":"65258"},"x_scale":{"id":"65262"},"y_range":{"id":"65260"},"y_scale":{"id":"65264"}},"id":"65257","subtype":"Figure","type":"Plot"},{"attributes":{"source":{"id":"65317"}},"id":"65321","type":"CDSView"},{"attributes":{},"id":"65305","type":"BasicTickFormatter"},{"attributes":{},"id":"65309","type":"UnionRenderers"},{"attributes":{"line_alpha":0.1,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"65295","type":"Line"},{"attributes":{"data_source":{"id":"65317"},"glyph":{"id":"65318"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"65319"},"selection_glyph":null,"view":{"id":"65321"}},"id":"65320","type":"GlyphRenderer"},{"attributes":{},"id":"65274","type":"ResetTool"},{"attributes":{"overlay":{"id":"65282"}},"id":"65276","type":"BoxZoomTool"},{"attributes":{"source":{"id":"65298"}},"id":"65302","type":"CDSView"},{"attributes":{"toolbars":[{"id":"65284"}],"tools":[{"id":"65274"},{"id":"65275"},{"id":"65276"},{"id":"65277"},{"id":"65278"},{"id":"65279"},{"id":"65280"},{"id":"65281"}]},"id":"65339","type":"ProxyToolbar"},{"attributes":{"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"65294","type":"Line"},{"attributes":{"fill_alpha":{"value":0.1},"fill_color":{"value":"#2a2eec"},"line_alpha":{"value":0.1},"size":{"units":"screen","value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"65319","type":"Circle"},{"attributes":{"data_source":{"id":"65293"},"glyph":{"id":"65294"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"65295"},"selection_glyph":null,"view":{"id":"65297"}},"id":"65296","type":"GlyphRenderer"},{"attributes":{},"id":"65280","type":"SaveTool"},{"attributes":{"axis":{"id":"65270"},"dimension":1,"ticker":null},"id":"65273","type":"Grid"},{"attributes":{"text":"y","text_font_size":{"value":"15pt"}},"id":"65322","type":"Title"},{"attributes":{},"id":"65258","type":"DataRange1d"},{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"65300","type":"Line"},{"attributes":{"bottom_units":"screen","fill_alpha":0.5,"fill_color":"lightgrey","left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","top_units":"screen"},"id":"65282","type":"BoxAnnotation"},{"attributes":{"data":{},"selected":{"id":"65334"},"selection_policy":{"id":"65333"}},"id":"65317","type":"ColumnDataSource"},{"attributes":{"children":[{"id":"65340"},{"id":"65338"}]},"id":"65341","type":"Column"},{"attributes":{},"id":"65277","type":"WheelZoomTool"},{"attributes":{"line_alpha":0,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"65299","type":"Line"},{"attributes":{"data_source":{"id":"65298"},"glyph":{"id":"65299"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"65300"},"selection_glyph":null,"view":{"id":"65302"}},"id":"65301","type":"GlyphRenderer"},{"attributes":{"active_drag":"auto","active_inspect":"auto","active_multi":null,"active_scroll":"auto","active_tap":"auto","tools":[{"id":"65274"},{"id":"65275"},{"id":"65276"},{"id":"65277"},{"id":"65278"},{"id":"65279"},{"id":"65280"},{"id":"65281"}]},"id":"65284","type":"Toolbar"},{"attributes":{},"id":"65275","type":"PanTool"},{"attributes":{"items":[{"id":"65316"}]},"id":"65315","type":"Legend"},{"attributes":{},"id":"65264","type":"LinearScale"},{"attributes":{"axis":{"id":"65266"},"ticker":null},"id":"65269","type":"Grid"},{"attributes":{"label":{"value":"bpv=0.53"},"renderers":[{"id":"65301"}]},"id":"65316","type":"LegendItem"},{"attributes":{},"id":"65333","type":"UnionRenderers"},{"attributes":{},"id":"65279","type":"UndoTool"},{"attributes":{"formatter":{"id":"65305"},"ticker":{"id":"65271"}},"id":"65270","type":"LinearAxis"},{"attributes":{},"id":"65312","type":"Selection"},{"attributes":{"toolbar":{"id":"65339"},"toolbar_location":"above"},"id":"65340","type":"ToolbarBox"},{"attributes":{"children":[[{"id":"65257"},0,0]]},"id":"65338","type":"GridBox"},{"attributes":{"data":{},"selected":{"id":"65312"},"selection_policy":{"id":"65311"}},"id":"65298","type":"ColumnDataSource"},{"attributes":{},"id":"65260","type":"DataRange1d"},{"attributes":{"callback":null},"id":"65281","type":"HoverTool"},{"attributes":{},"id":"65267","type":"BasicTicker"},{"attributes":{},"id":"65334","type":"Selection"},{"attributes":{"fill_color":{"value":"#2a2eec"},"size":{"units":"screen","value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"65318","type":"Circle"},{"attributes":{},"id":"65262","type":"LinearScale"},{"attributes":{"fill_alpha":0.5,"fill_color":"lightgrey","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"xs_units":"screen","ys_units":"screen"},"id":"65283","type":"PolyAnnotation"},{"attributes":{},"id":"65271","type":"BasicTicker"}],"root_ids":["65341"]},"title":"Bokeh Application","version":"2.2.1"}}';
                  var render_items = [{"docid":"fcd226ef-5b5c-419d-85da-7c0ee6316825","root_ids":["65341"],"roots":{"65341":"0d694408-e156-4e9d-83bc-32e6e30aaf1d"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    var attempts = 0;
                    var timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
        
        
        }
      ];
    
      function run_inline_js() {
        
        for (var i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();